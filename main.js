...
if (getQueryVariable("time")){
  ...
  var inputTime = getQueryVariable("time");
  inputTimeMinutes = Number(inputTime.split(":")[0]) * 60 + Number(inputTime.split(":")[1]);

  var sourceTimezone = decodeURIComponent(getQueryVariable("timezone").replace(/\+/g, " "));
  ...
  var totalOffset = Number(sourceOffset) - Number(localOffset);
  var finalTime = inputTimeMinutes + totalOffset;
